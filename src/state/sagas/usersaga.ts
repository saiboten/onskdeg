    ...
    firebase
      ...
      .once("value", snapshot => {
        ...
        if (resolveVal instanceof Array) {
          ...
          const userList = resolveVal
            .filter((el: User) => el.uid)
            .map((el: User) => el.uid);
  ...
  return new Promise((resolve, reject) => {
    firebase
      ...
      .once("value", snapshot => {
        ...
        const filteredList: Array<User> = userlist.filter(
          (user: User) => user.email === mail
        );
